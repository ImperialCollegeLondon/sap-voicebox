...
%      Version: $Id: v_readwav.m 10865 2018-09-21 17:22:45Z dmb $
%
...
if ischar(filename)
    ...
    info(1)=fid;
else                                % first argument is FIDX from previous call
    info=filename;
    fid=info(1);
end
...
if ~info(3)
    ...
    instlen=-1;
    info(13)=-1;
    info(12)='e';  % default is original wave file format
    while datalen<0						% loop until FMT and DATA chuncks both found
        ...
        if strcmp(header','fmt ')					% ******* found FMT chunk *********
            ...
            if len>16
                info(12)='x';  % might be WAVEFORMATEX format
            end
            ...
            if info(8)==-2   % wave format extensible
                ...
                info(12)='X';                         % WAVEFORMATEXTENSIBLE format
...
if nargin>2
    if nmax>=0
        ksamples=min(nmax,ksamples); % number of samples to read
    end
elseif ~getdat 
