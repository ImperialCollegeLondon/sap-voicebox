...
%      Version: $Id: v_kmeanhar.m 10865 2018-09-21 17:22:45Z dmb $
%
...
if ischar(x0)
    if k<n
        if any(x0=='p')                  % Initialize using a random partition
            ...
            ix(v_rnsubset(k,n))=1:k;      % but force at least one point per cluster
            x=zeros(k,p);
            for i=1:k
                x(i,:)=mean(d(ix==i,:),1);
            end
        else                                % Forgy initialization: choose k random points [default]
            x=d(v_rnsubset(k,n),:);         % sample k centres without replacement
        end
    else
