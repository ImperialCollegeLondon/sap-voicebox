...
%      Version: $Id: v_writewav.m 10865 2018-09-21 17:22:45Z dmb $
%
...
if any(mode=='e')
    wavtype=1;              % original WAVEFORMAT format
elseif any(mode=='x')
    wavtype=2;              % WAVEFORMATEX format
elseif any(mode=='X') || (nargin>=6 && ~isempty(mask) && mask~=0)
    wavtype=3;              % WAVEFORMATEXTENSIBLE format
else
    wavtype=2-(info(8)==1);
end
...
info(5)=nc;
if n                            % if there are any data values
    if sc~='r'                  % 'r' = no scaling
        if sc=='s'              % 's' = scale to peak signal
            if nargin>=7 && ~isempty(mad) && mad>0
                pd=mad;
            else
                pd=max(abs(d(:)));
