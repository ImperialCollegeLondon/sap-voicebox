function [c,qq,ff,tt,po]=v_modspect(s,fs,m,nf,nq,p)
%V_MODSPECT Calculate the modulation spectrum of a signal C=(S,FS,W,NC,P,N,INC,FL,FH)
%
%
% Simple use:
%
...
%   (4) Regarding each v_filterbank output as a time-varying signal power or amplitude [ppow;'pl'],
...
%      Version: $Id: v_modspect.m 10865 2018-09-21 17:22:45Z dmb $
%
...
if isempty(PP)
    ...
    PP.fmin=40;                                 % minimum mel v_filterbank frequency
    PP.fmax=10000;                              % maximum mel v_filterbank frequency (0 for Nyquist)
    PP.fwin='t';                                % mel v_filterbank filter shape: t=triangle, m=hamming, n=hann
    ...
    PP.mwin='t';                                % mel-mod v_filterbank filter shape: t=triangle, m=hamming, n=hann
...
%         w=hamming(nw+1)'; w(end)=[];        % Hamming window ([2] uses Hanning)
        w=0.54-0.46*cos(2*pi*(0:nw-1)/nw); % Hamming window ([2] uses Hanning)
    case 'n'
%         w=hanning(nw-1)';           % Hanning window: underlying period is nw
        w=0.5-0.5*cos(2*pi*(1:nw-1)/nw);  % Hanning window: underlying period is nw
    case 'r'
        w=ones(nw,1);                 % rctangular window
end
fx=v_rfft(v_enframe(s,w,ni),nf,2);
...
[mbk,ffm]=v_melbankm(po.fbin,nf,fs,po.fmin/fs,min(po.fmax/fs+(po.fmax==0),0.5),po.fwin);
...
mx=v_rfft(reshape(px(ix(:),:),mnw,nmt*npf),mnf,1);          % find modulation spectrum
% nmm=size(mx,1);             % number of modulation spectrum bins [not needed]
[qbk,qqm]=v_melbankm(po.mbin,mnf,100*fs/ni,po.mmin*ni/fs,min(po.mmax*ni/fs+(po.mmax==0),0.5),po.mwin);    % multiply frq by 100 to make it alsmost log
nqq=length(qqm);
switch po.mpow
    case 'a'
...
if po.qdcq
    dx=reshape(v_rdct(reshape(qx,nqq,nmt*npf)),nqq,nmt,npf);    % take dct in q direction
...
if po.qdcp
    dx=permute(reshape(v_rdct(reshape(permute(qx,[3,1,2]),npf,nqq*nmt)),npf,nqq,nmt),[2 3 1]);    % take dct in p direction
...
if bitand(po.dplt,512)
    ...
    ffx=repmat(v_mel2frq(ffm(:)),1,nqq)/1000;
