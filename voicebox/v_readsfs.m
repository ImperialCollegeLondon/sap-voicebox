...
%      Version: $Id: v_readsfs.m 10865 2018-09-21 17:22:45Z dmb $
%
...
if nargin<7
    xpath=v_voicebox('sfsbin');       % path for sfs programs
end
EXESUF=v_voicebox('sfssuffix');                                              % suffix for executable O/S dependent
...
if nargout==0
    if nargin<2 || ~numel(ty) || ty<=0
        [yy,fs,hd,ffx]=v_readsfs(ff,0,0,mode);
        fprintf(1,'File: %s\n',ffx{1});
        fprintf(1,'Database: %s, Speaker: %s, Date: %s, Token: %s\n',hd{6},hd{7},hd{9},hd{10});
        lst=ffx{3};
        for it=2:size(lst,1);
            [yy,fs,hd,ffx]=v_readsfs(ffx,lst(it,1),lst(it,2),mode,0);
            nf=hd{1}(7);
            fd=hd{1}(9);
            fprintf(1,'%3d.%02d %ss @ %sHz (%d frames): %s\n',lst(it,1),lst(it,2),v_sprintsi(nf*fd),v_sprintsi(1/fd),nf,ffx{4}{it,1});
        end
    end
else
    it=[];
    ...
    while (isempty(it))                 % may go round this loop twice
        if ischar(ff)           % If ff is a string we must read file
            if any(mode=='d')
                ff=fullfile(v_voicebox('dir_data'),ff);
            end
            fid=fopen(ff,'rb','b');
    ...
    if ~lit(1)                              % read main header
        ...
        if nargout>2
            ...
            for i=1:13
                hd{i+1}=char(v_zerotrim(mb(mc(i,1):mc(i,2))));
            end
        end
    else
