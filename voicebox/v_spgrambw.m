...
%      Version: $Id: v_spgrambw.m 10865 2018-09-21 17:22:45Z dmb $
%
...
if ~numel(fmax)             % no explicit frequency range
    switch mdsw(1)
        ...
        case 'm'
            fx=linspace(0,v_frq2mel(fnyq),nfrq);   % DC to Nyquist
        case 'b'
            fx=linspace(0,v_frq2bark(fnyq),nfrq);   % DC to Nyquist
        case 'e'
            fx=linspace(0,v_frq2erb(fnyq),nfrq);   % DC to Nyquist
    ...
    if any(mode=='h')
        switch mdsw(1)
            ...
            case 'm'
                fmaxu=v_frq2mel(fmax);   % DC to Nyquist
            case 'b'
                fmaxu=v_frq2bark(fmax);   % DC to Nyquist
            case 'e'
                fmaxu=v_frq2erb(fmax);   % DC to Nyquist
            otherwise
...
if ns2==1   % input is a speech signal vector
    ...
    [sf,t]=v_enframe(s(ix1:ix2),win,ninc);
    t=fs(2)+(t+ix1-2)/fs(1);                         % time axis
    b=v_rfft(sf,fftlen,2);
    ...
    fftlen=[ns2 fs(3) fs(4)]; % for v_filtbankm: ns2=# input freq bins, freq increment (Hz), first bin freq (Hz)
    fftfs=0;
...
if ~nargout || any(mode=='g')
    ...
    if any(mode=='f') && ~strcmp(frlab,'Hz')
        ...
        ylabel(['Frequency (' v_yticksi frlab ')']);
    end
    ytick=get(gca,'YTick');
