function d=v_distisar(ar1,ar2,mode)
%V_DISTISAR calculates the Itakura-Saito distance between AR coefficients D=(AR1,AR2,MODE)
%
...
%
%         np=255; d0=v_distisar(ar1,ar2); d1=v_distispf(v_lpcar2pf(ar1,np),v_lpcar2pf(ar2,np))
%
% Autocorrelation LPC analysis is equivalent to minimizing the Itakura-Saito difference between the
% signal spectrum and that of the all-pole LPC filter, i.e. v_distispf(pf,v_lpcar2pf(ar0,np)).
...
%
%               v_distispf(pf,v_lpcar2pf(ar,np)) = v_distispf(pf,v_lpcar2pf(ar0,np)) + v_distisar(ar0,ar)
...
%      Version: $Id: v_distisar.m 10865 2018-09-21 17:22:45Z dmb $
%
...
if any(mode=='d') | (mode~='x' & nf1==nf2)
   nx=min(nf1,nf2);
   d=2*sum(v_lpcar2rr(ar1(1:nx,:)).*m2(1:nx,:),2)-log((ar2(1:nx,1)./ar1(1:nx,1)).^2)-1;;
else
   d=2*v_lpcar2rr(ar1)*m2'-log((ar1(:,1).^(-1)*ar2(:,1)').^2)-1;
end
