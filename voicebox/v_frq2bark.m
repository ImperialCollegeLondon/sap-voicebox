function [b,c] = v_frq2bark(f,m)
%V_FRQ2BARK  Convert Hertz to BARK frequency scale BARK=(FRQ)
...
%   band ceil(v_frq2bark(f)). The inverse function is v_bark2frq.
%
...
%       b1=v_frq2bark(f0);      b2=v_frq2bark(f0,'lh');
%       b3=v_frq2bark(f0,'LH'); b4=v_frq2bark(f0,'z');
%       plot(b0,[b0 b1 b2 b3 b4]-repmat(b0,1,5));
%       xlabel('Frequency (Bark)'); ylabel('Error (Bark)');
%       legend('Exact','v_voicebox','Traunmuller1990', ...
%              'Traunmuller1983','Zwicker1980','Location','South');

%      Copyright (C) Mike Brookes 2006-2010
%      Version: $Id: v_frq2bark.m 10865 2018-09-21 17:22:45Z dmb $
%
...
if ~nargout || any(m=='g')
    ...
    semilogy(f,c,'-r');
    ha=gca;
    ylabel(['Critical BW (' v_yticksi 'Hz)']);
