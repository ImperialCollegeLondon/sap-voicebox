...
% spyax contains the y axis label
...
%        for i=1:length(rb), fprintf(' %s',v_sprintcpx(rb(i),'0.14gi')); end
%        for i=1:length(ra), fprintf(' %s',v_sprintcpx(ra(i),'0.14gi')); end, fprintf('];\n');
if isempty(spz)
    spty={'White';'A-Weight';'B-Weight';'C-Weight';'X1-LTASS-P50';'X1-LTASS-1994';'SII-IntInv';'BS-468';'USASI';'POTS';'LTASS-P50';
        'X2-LTASS-1994';'LTASS-1994';'EM3346-Gain';'EM3346-Noise'};
    nspty=size(spty,1);
    spyax={'';'';'';'';'dB-SPL/\surd{}Hz @ 25 mm';'';'SII-IntInv';'BS-468';'USASI';'POTS';'dB-SPL/\surd{}Hz @ 1m'; '';
        'dB-SPL/\surd{}Hz @ 1m';'dB-V/SPL';'dB-V/\surd{}Hz'};
    spz={[1 0];
        ...
        [-2.309644968403e+23 2 0 -58249.19407694 -23164.744808779+191400.50379156i -34701.140307771+126665.52643837i -11829.456694949+32636.504600476i -33607.63134518 -991.35267796254];
...
if (nargin<3 || ~numel(f)) && any(m1=='fmpd') % calcualte the frequency range
    ...
    f=logspace(log10(min(apz)*0.5/pi)-0.5,log10(max(apz)*0.5/pi)+0.5,200);
end
if any(m1=='fmpdle')
...
switch m1
    ...
    case 't'
        if nargin<5 || ~numel(zi)
            [b,a]=v_randfilt(bz,az,n);
        else
            [b,a]=v_randfilt(bz,az,n,zi);
        end
        b=b*10*log10(fs/2); % scale to compensate for bandwidth
    case 'i'
        if nargin<5 || ~numel(zi)
...
if ~nargout || ~strcmp(m,lower(m))
    ...
    if any(m=='M') || any(m=='Q') || any(m=='E') || any(m=='E') % draw a frequency response plot
        ...
        if pam                              % plot a magnitude response
            ...
            v_texthvc(0.02,0.98,pwrint,'LTk');
            xlabel(['Frequency (' v_xticksi 'Hz)']);
            if si>0 && ~isempty(spyax{si})
                ylabel(spyax{si});
            else
                ylabel('Gain (dB)');
            end
        ...
        if pae % plot magnitude response error
            if nax>1
                subplot(nax,1,1+pam);
            end
            if any(m=='F')
            ...
            v_texthvc(0.95,0.95,sprintf('Err < %.1f dB%s',max(abs(dberr)),dbflt),'RTk');
            if si>0 && ~pam
                title(sprintf('Type %d: %s%s',si,spty{si},titex));
            end
            axh(end+1)=gca;
