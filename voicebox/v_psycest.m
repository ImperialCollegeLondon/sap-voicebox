...
%      Version: $Id: v_psycest.m 10865 2018-09-21 17:22:45Z dmb $
%
...
% (3) v_entropy seems to change signifiacntly when the grid is rescaled
...
elseif iq>0
    ...
    if nargin>=3
        ...
        hn=hn*hfact;         % increase v_entropy gains to ensure all models get a chance
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Calculate mean and covariance and v_entropy  %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if sm>1 && sm<nsq                           % use quadratic interpolation in log prob if possible
        [dum,sm2]=v_quadpeak(log(ps(sm-1:sm+1)));
        sm=sm+sm2-2;
    end
    ...
    xh=(px*log(px)')*(xqi(1)-xqi(2));           % differential v_entropy h(x0)
    sh=(ps'*log(ps))*(sqi(1)-sqi(2));          	% differential v_entropy h(s0)
    if nargin==3
        switch nr(9) % cost function: 1=variance, 2=v_entropy
            case 1
                ...
                if any(res(nresr+1:nres,1)==jq)   % if we have had any recent probe results for this model
                    ...
                    if any(mou) % if any results must be excluded
                        ...
                        if smr>1 && smr<nsq                           % use quadratic interpolation in log prob if possible
                            [dum,sm2]=v_quadpeak(log(psr(smr-1:smr+1)));
                            smr=smr+sm2-2;
                        end
        ...
        for i=1:length(po)
            ...
            switch poi
                case 'p'    % plot pdf
                    ...
                    axis 'xy';
                    %     colorbar;
                    %     v_cblabel('Relative Probability');
